...
async def create_shopify_charge(fullz , session):
    try:
        ...
        g = await session.get(f'{checkout_url}/processing?from_processing_page=1&validate=true')
        url_g = str(g.url)
        url_g = await session.get(url_g)

        # print(url_g.text)

        # amount_capture = find_between(url_g.text, 'value: "', '",')

        # print(amount_capture)
